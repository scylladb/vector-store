...
def upload_generated_files(generated_dir):
    ips = [f for f in os.listdir(generated_dir) if os.path.isdir(f) and f not in ('.', '.. ')]
    for ip in ips:
            ...
            if not apps or service in apps:
                if service == "scylla":
                    for ip in z.get("scylla", []):
                        upload(ip, "common" + "/*")
                        upload(ip, service + "/*")  # upload all files for the role
...
def generate_monitor_config(base_dir, zones, params):
    ...
    for z in zones:
        for ip in z.get("scylla", []):
            scylla_nodes.append(ip)
        v = z.get("vector")
        if v:
            vector_nodes.append(v)
    ...
    env_path = path_join(base_dir, "monitor", "env.sh")
...
def generate_vector_config(base_dir, zones, generated_dir, params):
    ...
    for z in zones:
        if "vector" in z and "scylla" in z:
            run_path = path_join(base_dir, generated_dir + "/" + z["vector"], "start-cmd.sh")
            os.makedirs(path_join(base_dir, generated_dir, z["vector"]), exist_ok=True)
            with open(run_path, "w", encoding="utf-8") as f:
                f.write("#!/bin/bash\n")
                f.write("cd /home/ubuntu/vector-store\n")
                f.write("export VECTOR_STORE_URI=0.0.0.0:6080\n")
                f.write("export VECTOR_STORE_SCYLLADB_URI={IP}:9042\n".format(IP=z["scylla"][0]))
    ...
    for z in zones:
        if "vector" in z and "scylla" in z:
            for ip in z["scylla"]:
                if not seed:
                    seed = ip
                compose = path_join(base_dir, generated_dir, ip, "docker-compose.yml")
                os.makedirs(path_join(base_dir, generated_dir, ip), exist_ok=True)
                commands["--vector-store-primary-uri"]  = "http://{IP}:6080".format(IP=z["vector"])
    ...
    for z in zones:
        ...
        for service in ["client", "vector", "monitor"]:
            if service in z and z[service] and (not apps or service in apps):
                trace_verbose(f"Installing {service} on {z[service]}")
                run_remote(z[service], 'install-python.sh')
                run_remote(z[service], 'install.sh')
...
def stage_run_load(zones):
    ...
    for z in zones:
        for ip in z.get("client", []):
            ...
            run_remote(ip, "bash run_test.sh")
