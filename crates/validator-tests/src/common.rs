...
pub async fn create_keyspace(session: &Session) -> String {
    ...
    // Create keyspace with datacenter-based replication 3 for the 3-node cluster
    session.query_unpaged(
        format!("CREATE KEYSPACE {keyspace} WITH replication = {{'class': 'NetworkTopologyStrategy', 'datacenter1': 3}}"),
        (),
