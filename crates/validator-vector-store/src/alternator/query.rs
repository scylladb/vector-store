...
async fn query_uses_selected_vector_index(actors: TestActors) {
    ...
    let lower_index = vector_store::IndexInfo::new(
        super::alternator_keyspace(&table_name).as_ref(),
        lower_index_name.as_ref(),
    );
    let upper_index = vector_store::IndexInfo::new(
        super::alternator_keyspace(&table_name).as_ref(),
    ...
    let lower_items = client
        ...
        .to_vec();
    assert_eq!(
        extract_s(&lower_items[0], partition_key_name),
        "pk-a",
    ...
    let upper_items = client
        ...
        .to_vec();
    assert_eq!(
        extract_s(&upper_items[0], partition_key_name),
        "pk-c",
...
///
/// The test uses a retry loop (similar to nyh's upstream Python tests) to
/// tolerate the short window between the index reaching SERVING state and all
/// items being fully queryable.
#[framed]
async fn query_with_vector_search_multiple_results_ordering(actors: TestActors) {
    ...
    let expected_order: Vec<String> = dataset
        .iter()
        .map(|i| match i.0.get("Pk-Ord").expect("item has no Pk-Ord") {
            AttributeValue::S(s) => s.clone(),
            other => panic!("expected S pk, got {other:?}"),
        })
        .collect();

    // Retry loop: the index may need a moment to return results in correct
    // order after all items are indexed (mirrors nyh's upstream Python tests).
    info!(
        "Issuing Query with VectorSearch Limit=5 on '{}' (with retry)",
        ctx.table_name
    );
    ...
    loop {
        ...
        last_items = raw
            .iter()
            .map(|item| extract_s(item, "Pk-Ord"))
            .collect::<Vec<_>>();
        if last_items == expected_order {
            break;
        }
    ...
    assert_eq!(
        last_items.len(),
        5,
        "Query should return all 5 items, got {}",
        last_items.len()
    );
    assert_eq!(
        last_items, expected_order,
...
async fn query_with_projection_special_names(actors: TestActors) {
    ...
    for shape in super::NAME_PATTERNS {
        ...
        let item = &items[0];
        assert!(
            item.contains_key(pk_name),
            "projected item should contain partition key '{pk_name}', got keys: {:?}",
            item.keys().collect::<Vec<_>>()
        );
        if let Some(sk_name) = shape.sk {
            assert!(
                item.contains_key(sk_name),
                "projected item should contain sort key '{sk_name}', got keys: {:?}",
                item.keys().collect::<Vec<_>>()
            );
        }
        assert!(
            !item.contains_key(vec_name),
            "projected item should NOT contain vector attribute '{vec_name}', got keys: {:?}",
            item.keys().collect::<Vec<_>>()
        );
        assert!(
            !item.contains_key(data_attribute_name),
            "projected item should NOT contain data attribute '{data_attribute_name}', got keys: {:?}",
            item.keys().collect::<Vec<_>>()
        );
...
async fn query_with_select_all_attributes(actors: TestActors) {
    ...
    let item = &items[0];
    assert!(
        item.contains_key(ctx.pk.as_str()),
        "AllAttributes should return partition key"
    );
    assert!(
        item.contains_key(vector_attribute_name),
...
async fn query_with_key_condition_expression(actors: TestActors) {
    ...
    assert_eq!(
        items.len(),
        2,
        "KeyConditionExpression should return both pk-a items (got {})",
        items.len()
    );
    for item in items {
        assert_eq!(
            extract_s(item, pk_name),
            "pk-a",
            "all returned items must satisfy the KeyConditionExpression (pk = 'pk-a')"
        );
    }
    assert!(
        items.iter().all(|i| extract_s(i, pk_name) != "pk-b"),
        "pk-b items must be excluded by the KeyConditionExpression"
    );
...
async fn query_with_filter_expression(actors: TestActors) {
    ...
    assert_eq!(
        items.len(),
        2,
        "FilterExpression should keep only the 2 'keep' items (got {})",
        items.len()
    );
    for item in items {
        assert_eq!(
            extract_s(item, cat_name),
            "keep",
            "all returned items must satisfy the FilterExpression (Category = 'keep')"
        );
    }
    assert!(
        items.iter().all(|i| extract_s(i, pk_name) != "pk-drop"),
        "'pk-drop' must be excluded by the FilterExpression"
    );
    // ANN ordering is preserved after filtering: pk-keep-1 is nearest to
    // [1,1,1] (exact match), pk-keep-2 is slightly farther.
    assert_eq!(
        extract_s(&items[0], pk_name),
        "pk-keep-1",
        "nearest 'keep' item should be pk-keep-1"
    );
    assert_eq!(
        extract_s(&items[1], pk_name),
        "pk-keep-2",
        "second 'keep' item should be pk-keep-2"
    );
