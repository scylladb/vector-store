...
async fn test_decimal_key(actors: TestActors) {
    ...
    // CK overwrite: ck=3.140 overwrites ck=3.14 in pk=1.0.
    // PK byte-identity: pk=1.00 is a separate partition.
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("1.00".to_string(), "3.14".to_string()),
        ],
        "full scan: PK byte-identity preserved, CK normalized to 3.14"
    );

    // Phase 2: after index creation (CDC path).
    // Same pattern: CK overwrite + PK byte-identity.
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("2.00".to_string(), "7.5".to_string()),
        ],
        "CDC: PK byte-identity preserved, CK normalized"
    );
