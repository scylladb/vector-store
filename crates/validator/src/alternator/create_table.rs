...
impl Intercept for VectorIndexesAssertInterceptor {
    ...
    fn read_after_deserialization(
        ...
        let table = json.get("Table").ok_or(format!(
            "raw DescribeTable should contain 'Table' key ({ctx})"
        ))?;

        let vector_indexes = table
            .get("VectorIndexes")
            .ok_or(format!(
                "Table should contain 'VectorIndexes' extension field ({ctx})"
            ))?
            .as_array()
            .ok_or(format!("VectorIndexes should be an array ({ctx})"))?;
        ...
        let va = vi
            ...
            .ok_or(format!(
                "VectorIndex should contain VectorAttribute object ({ctx})"
            ))?;
        assert_eq!(
            (
            ...
            "VectorAttribute should match ({ctx})"
        );

        // After wait_for_index completes, the index must report ACTIVE.
        assert_eq!(
            vi.get("IndexStatus").and_then(|v| v.as_str()),
            Some("ACTIVE"),
            "VectorIndex should be ACTIVE after wait_for_index ({ctx})"
        );
