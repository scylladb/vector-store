...
pub async fn init_with_config(
    ...
    // Only bootstrap the default role when the cluster was actually started
    // with the default auth config (i.e. the shared superuser identity is
    // valid for it). Callers that bring their own auth setup (e.g. dedicated
    // auth tests) opt out by using a different `extra_config`.
    let use_default_auth = scylla_configs
        .iter()
        .all(|config| config.extra_config.as_deref() == Some(scylla_auth_config().as_slice()));
