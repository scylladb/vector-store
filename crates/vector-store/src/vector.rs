...
/// - `0x05` (vector): optimized sequential 32-bit big-endian floats.
/// - `0x04` (list): unoptimized JSON representing List values.
fn parse_alternator_vector(bytes: &[u8]) -> anyhow::Result<Vec<f32>> {
    match bytes.first() {
        Some(&ALTERNATOR_TYPE_VECTOR) => parse_alternator_vector_binary(&bytes[1..]),
...
mod tests {
    ...
    /// Prepend the Alternator JSON type tag (0x04) to a DynamoDB JSON string,
    ...
    /// Prepend the Alternator Vector type tag (0x05) to a sequence of big-endian floats,
    /// mirroring how Alternator serialises the optimized Vector type.
